...
class Trainer:
    ...
    def __init__(
        ...
        self.args = args
        self.train_dataset_index = train_dataset_index
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # if indices:
        #     assert len(outputs['loss_no_reduction'])==len(indices),\
        #         f"{len(outputs['loss_no_reduction'])},{len(indices)}"
        #     for l,idx in zip(outputs['loss_no_reduction'],indices):
        #         # for param in model.parameters():
        #         #     # a = param.grad.data
        #         #     print('gradient ', param.grad)
        #         #     # print(torch.sum(a))
        #         #     print('parameters: ', param[0][:5])
        #         #     break
        #         model.zero_grad()
        ...
        #                 # print('cur gradients: ',torch.sum(cur_grad).item())
        #                 cur_gradients += torch.sum(torch.abs(cur_grad)).item()
        #
        #                 # break
        #             # break
        #         # print(self.optimizer.calculate_updates())
        #         self.train_dataset_index[idx]['gradients'].append(cur_gradients)
        #         self.train_dataset_index[idx]['updates'].append(self.optimizer.calculate_updates())
        #
        #     model.zero_grad()
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # print(inputs)
            if 'idx' in inputs:
                cur_idx = inputs['idx'].tolist()
            loss, logits, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            if self.data_args is not None and self.data_args.confidence_prediction:
                my_probs = logits[0].cpu().tolist()
                assert len(cur_idx)==len(my_probs)
                for idx,p in zip(cur_idx,my_probs):
                    confidence_scores.append([idx, max(p)])
        ...
        if self.data_args is not None and self.data_args.confidence_prediction:
            import json
            sorted_probs = sorted(confidence_scores, key=lambda item: item[1])
            added_indices = [pair[0] for pair in sorted_probs[:self.data_args.acquisition_batch_size]]
            with open(os.path.join(self.data_args.selected_indices_store_path, f"confidence_{self.data_args.tag}.json")) as f:
                pre_selected_indices = json.load(f)
            with open(os.path.join(self.data_args.selected_indices_store_path, f"confidence_{self.data_args.tag}.json"),'w') as f:
                json.dump(pre_selected_indices+added_indices,f)
            ...
            if is_sagemaker_mp_enabled():
                ...
                if has_labels:
                    ...
                    if isinstance(outputs, dict):
                        logits = tuple(v for k, v in outputs.items() if k not in ignore_keys + ["loss",'loss_no_reduction'])
                    else:
                        logits = outputs[1:]
                else:
                    loss = None
