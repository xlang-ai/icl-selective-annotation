...
class AdamW(Optimizer):
    ...
    def step(self, closure: Callable = None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # if count==0:
                #     print('p before: ',p[0])
                #     print('update ',(exp_avg/denom*(-step_size))[0])
    ...
    def calculate_updates(self, closure: Callable = None):
        """
        Performs a single optimization step.
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                updates += torch.sum(torch.abs(exp_avg/denom*(-step_size))).item()
                ...
                # Add weight decay at the end (fixed version)
                if group["weight_decay"] > 0.0:
                    raise ValueError('weight decay needs to adjust such that to add total absolute value')
                    updates += torch.sum((p.data+exp_avg/denom*(-step_size))*(-group["lr"] * group["weight_decay"])).item()
                    # p.data.add_(p.data, alpha=(-group["lr"] * group["weight_decay"]))
