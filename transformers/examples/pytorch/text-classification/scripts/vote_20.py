...
for seed in [2,4,12,24,32,36,38,42,56,58]:
    os.system(f"python run_glue.py --model_name_or_path roberta-base --task_name mrpc --do_train --do_eval "
              ...
              f"--output_dir /nvme/.cache/outputs/mrpc_vote_20_select --annotation_size 100 --seed {seed} "
              f"--train_emb_dir /nvme/.cache/embeddings "
              f"--sentence_transformer_model sentence-transformers/all-mpnet-base-v2 "
              f"--selection_method vote_20_select --overwrite_output > outputs/vote_20_select_{seed}.txt")
    try:
        if os.path.isdir(f'/nvme/.cache/outputs/mrpc_vote_20_select'):
            shutil.rmtree(f'/nvme/.cache/outputs/mrpc_vote_20_select')
    except:
        pass