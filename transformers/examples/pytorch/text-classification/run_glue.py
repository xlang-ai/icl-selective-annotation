...
def acc_and_f1_dbpedia(preds, labels):
...
class DataTrainingArguments:
    ...
    annotation_size: int = field(default=-1,metadata={"help": "annotation_size"},)
    overlap_threshold: float = field(default=0.5, metadata={"help": "overlap_threshold"}, )
...
def calculate_sentence_transformer_embedding(examples,embedding_model,task_name,mean_normal=False):
    if task_name in ['mrpc']:
        text_to_encode = ["{}.\n{}".format(raw_item["sentence1"], raw_item["sentence2"]) for raw_item in examples]
    ...
    if training_args.do_train:
        ...
        if data_args.selection_method is not None:
            if data_args.selection_method in ['random']:
                ...
                # elif data_args.selection_method in ['vote_1']:
